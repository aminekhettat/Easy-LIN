...
class CommunicationPanel(ttk.LabelFrame):
    ...
    def _build_connection_bar(self) -> None:
        ...
        self._connect_btn = ttk.Button(
            bar, text='Connect', command=self._on_connect
        )
        ...
        ttk.Button(
            bar, text='Clear Log', command=self._clear_log
    ...
    def _build_log_table(self) -> None:
        ...
        col_widths = {'#': 50, 'Time': 100, 'Dir': 40, 'ID': 60,
                      'Data': 200, 'Name': 140}
        for col in cols:
    ...
    def _refresh_frame_combo(self) -> None:
        ...
        if self._ldf:
            names = sorted(
                f'{frm.name}  (0x{frm.frame_id:02X})'
                for frm in self._ldf.frames.values()
            )
    ...
    def _on_connect(self) -> None:
        ...
        if self._bus.is_simulation:
            self._status_var.set('⚠  Simulation mode (no Vector hardware)')
            self._status_label.config(foreground='#d68910')
        else:
            self._status_var.set(
                f'Connected  ch={channel}  {bitrate} bps'
            )
    ...
    def _on_frame_selected(self, _event: tk.Event) -> None:
        ...
        if m:
            ...
            if frame_name in self._ldf.frames:
                frm = self._ldf.frames[frame_name]
                self._send_data_var.set(' '.join(['00'] * frm.length))
